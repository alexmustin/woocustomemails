...
class Custom_Emails_Per_Product_For_WC_Output {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_order_status_pending_to_on-hold_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );
		add_action( 'woocommerce_order_status_failed_to_on-hold_notification', array( $this, 'status_action_onhold' ), 10, 2 );
		add_action( 'woocommerce_order_status_failed_to_on-hold_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );
		add_action( 'woocommerce_order_status_cancelled_to_on-hold_notification', array( $this, 'status_action_onhold' ), 10, 2 );
		add_action( 'woocommerce_order_status_cancelled_to_on-hold_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );

		// PROCESSING STATUS.
		add_action( 'woocommerce_order_status_cancelled_to_processing_notification', array( $this, 'status_action_processing' ), 10, 2 );
		add_action( 'woocommerce_order_status_cancelled_to_processing_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );
		add_action( 'woocommerce_order_status_failed_to_processing_notification', array( $this, 'status_action_processing' ), 10, 2 );
		add_action( 'woocommerce_order_status_failed_to_processing_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );
		add_action( 'woocommerce_order_status_on-hold_to_processing_notification', array( $this, 'status_action_processing' ), 10, 2 );
		add_action( 'woocommerce_order_status_on-hold_to_processing_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );
		add_action( 'woocommerce_order_status_pending_to_processing_notification', array( $this, 'status_action_processing' ), 10, 2 );
		add_action( 'woocommerce_order_status_pending_to_processing_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );

		// COMPLETED STATUS.
		add_action( 'woocommerce_order_status_completed_notification', array( $this, 'status_action_completed' ), 10, 2 );
		add_action( 'woocommerce_order_status_completed_notification', array( $this, 'cepp4wp_insert_content' ), 10, 2 );
	 ...
	 */
	public function cepp4wp_insert_content( $shown_messages, $order ) {
		...
		if ( ! function_exists( 'cepp4wp_output_message' ) ) {
			 ...
			 * @param array   $shown_messages An array of which messages have already been added to the email.
			 */
			function cepp4wp_output_message( $order, $sent_to_admin, $shown_messages ) {
				...
				foreach ( $items as $item ) {
					...
					$cepp4wc_message_id         = get_post_meta( $this_product_id, 'cepp4wc_message_id', true );
					$templatelocation_meta = get_post_meta( $this_product_id, 'location', true );

					// Assign to different statuses.
					$cepp4wc_message_id_onhold           = (int) get_post_meta( $this_product_id, 'cepp4wc_message_id_onhold', true );
					$cepp4wc_message_location_onhold     = get_post_meta( $this_product_id, 'location_onhold', true );
					$cepp4wc_message_id_processing       = (int) get_post_meta( $this_product_id, 'cepp4wc_message_id_processing', true );
					$cepp4wc_message_location_processing = get_post_meta( $this_product_id, 'location_processing', true );
					$cepp4wc_message_id_completed        = (int) get_post_meta( $this_product_id, 'cepp4wc_message_id_completed', true );
					$cepp4wc_message_location_completed  = get_post_meta( $this_product_id, 'location_completed', true );
					...
					if ( ! empty( $cepp4wc_message_id_onhold ) || ! empty( $cepp4wc_message_id_processing ) || ! empty( $cepp4wc_message_id_completed ) ) {

						// ON-HOLD Status.
						if ( 'woocommerce_order_status_on-hold' === $this_order_status_action ) {

							// If there is an email assigned for 'On hold' status...
							if ( ! empty( $cepp4wc_message_id_onhold ) ) {

								// If this message has not already been shown in this email...
								if ( ! in_array( $cepp4wc_message_id_onhold, $shown_messages, true ) ) {
									...
									if ( $cepp4wc_message_location_onhold === $this_email_template_location ) {
										...
										$output .= nl2br( get_post_field( 'post_content', $cepp4wc_message_id_onhold ) );
										...
										$shown_messages[] = $cepp4wc_message_id_onhold;
						...
						if ( 'woocommerce_order_status_processing' === $this_order_status_action ) {
							...
							if ( ! empty( $cepp4wc_message_id_processing ) ) {

								// If this message has not already been shown in this email...
								if ( ! in_array( $cepp4wc_message_id_processing, $shown_messages, true ) ) {
									...
									// If message location setting is equal to the current email template location...
									if ( $cepp4wc_message_location_processing === $this_email_template_location ) {
										...
										// Output the_content of the saved WCE Message ID.
										$output .= nl2br( get_post_field( 'post_content', $cepp4wc_message_id_processing ) );
										...
										// Update 'shown_emails' var.
										$shown_messages[] = $cepp4wc_message_id_processing;
						...
						if ( 'woocommerce_order_status_completed' === $this_order_status_action ) {
							...
							if ( ! empty( $cepp4wc_message_id_completed ) ) {

								// If this message has not already been shown in this email...
								if ( ! in_array( $cepp4wc_message_id_completed, $shown_messages, true ) ) {
									...
									// If message location setting is equal to the current email template location...
									if ( $cepp4wc_message_location_completed === $this_email_template_location ) {
										...
										// Output the_content of the saved WCE Message ID.
										$output .= nl2br( get_post_field( 'post_content', $cepp4wc_message_id_completed ) );
										...
										// Update 'shown_emails' var.
										$shown_messages[] = $cepp4wc_message_id_completed;
							...
							if ( $this_order_status_action === $orderstatus_meta ) {
								...
								if ( ! in_array( $cepp4wc_message_id, $shown_messages, true ) ) {
									...
									if ( $this_email_template_location === $templatelocation_meta ) {
										...
										// Output the_content of the saved WCE Message ID.
										$output .= nl2br( get_post_field( 'post_content', $cepp4wc_message_id ) );
										...
										// Update 'shown_emails' var.
										$shown_messages[] = $cepp4wc_message_id;
		...
		add_action( 'woocommerce_email_before_order_table', 'cepp4wp_output_message', 10, 3 );
		add_action( 'woocommerce_email_after_order_table', 'cepp4wp_output_message', 10, 3 );
		add_action( 'woocommerce_email_order_meta', 'cepp4wp_output_message', 10, 3 );
		add_action( 'woocommerce_email_customer_details', 'cepp4wp_output_message', 10, 3 );
